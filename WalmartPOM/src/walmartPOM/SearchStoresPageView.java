...
public class SearchStoresPageView extends WalmartBaseView {
	...
	public SearchStoresPageView(RemoteWebDriver driver) throws Exception{
        ...
        try {
        	PerfectoUtils.fluentWait(pageText, driver, 20);
        	Thread.sleep(4000);
		} catch (Exception e) {
	...
	public SearchStoresPageView searchAddress(String address) throws IOException{
		...
		//search address 
		try {
			this.driver.findElement(storeLocatorSearchArea).sendKeys(address);
			this.driver.findElement(goButton).click();
			this.driver.getKeyboard().pressKey(Keys.ENTER);
			try {
				PerfectoUtils.fluentWait(resultsPage, this.driver, 20);
			} catch (Exception e) {
