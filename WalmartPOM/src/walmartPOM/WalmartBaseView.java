...
public class WalmartBaseView {
	...
	public WalmartBaseView init() throws IOException{
		...
		try{
			PerfectoUtils.fluentWait(walmartLogo, driver, 20);
		}
		catch(Exception e){
	...
	public SearchItemsPageView searchItem(String item) throws IOException{
		...
		try {
			...
			this.driver.findElement(searchArea).sendKeys(item);
	 ...
	 * @throws IOException 
	 **********************************************************************/
	public SearchStoresPageView clickLocateStores() throws IOException{
		
		try {
			//click on locate stores button
			this.driver.findElement(storeLocatorButton).click();
	...
	public WalmartBaseView clickHomePage(){
		try {
			//click on locate stores button
			this.driver.findElement(walmartLogo).click();
